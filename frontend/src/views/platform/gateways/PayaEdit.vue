<template>
  <div class="content">
    ...
    />
    <h1>Gateway - Paya API url-<span v-text="form.name"></span></h1>
    <div>
...
const form = reactive<{ id: string | null, name: string, base_url: string }>({
  id: null,
  name: "",
  base_url: "",
});
...
const handleSubmit = async () => {
  try {
    const id = form.id;
    const response = await post(`gateways/paya/${id || ""}`, form);
