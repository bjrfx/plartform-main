...
const headers = [
  ...
  {name: 'Merchant Name', key: 'merchant', sortable: false},
...
const selectedMerchants = ref<string[]>(
    Array.isArray(route.query?.merchants)
        ? (route.query.merchants.filter((item): item is string => typeof item === 'string'))
        : route.query?.merchants
            ? [String(route.query.merchants)]
            : []
);
...
const initiateFilter = () => {
  ...
  if (selectedMerchants.value) {
    query.merchants = selectedMerchants.value;
  }
